        ...
        ((=)
         (let ((v (decl-var (cadr e)))
           ...
           (if (or (ssavalue? v) (globalref? v) (underscore-symbol? v))
               ...
               (cons v rest))))
        (else
         (apply append! (map find-assigned-vars
                             e))))))
  ...
  (cond ((symbol? e)
        ...
        ((eq? (car e) 'scope-block)
         (let* ((blok (cadr e)) ;; body of scope-block expression
                ...
                (vars-def (find-local-def-decls blok) '())
                (locals-declared (find-local-decls blok) vars-def)
                ...
                (implicit-locals
                 (filter (if toplevel
                             ...
                             (lambda (v) (and (memq (var-kind v scope) '(none static-parameter))
                                              (not (memq v vars-def))
                                              (not (memq v locals-declared))
                                              (not (memq v globals)))))
                         (find-assigned-vars blok)))
                (vars (delete-duplicates (append locals-declared implicit-locals)))
           ...
           (for-each (lambda (v)
                       (if (or (memq v vars-def) (memq v locals-declared))
           ...
           (if (eq? e (lam:body lam))
               (for-each (lambda (v)
                           (if (or (memq v vars-def) (memq v locals-declared) (memq v implicit-locals))
